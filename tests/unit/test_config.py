...
@pytest.mark.parametrize('keys, default, type, expected', [
    ...
    (['Characters', 'Professor X', 'powers and abilities'], None, config.List(config.Str), ['telepathy', 'intelligence']),
])
...
@pytest.mark.parametrize('keys, default, type', [
    ...
    (['Characters', 'Professor X', 'powers and abilities'], None, config.List(config.Int)),
])
...
@pytest.mark.parametrize('const_value, test_input, expected', [
    ('test', 'test', True),
    ('test', 'not test', False),
    ('test', 10, False),
    (10, 10, True),
...
@pytest.mark.parametrize('choices, test_input, expected', [
    (['a', 'b', 'c'], 'b', True),
    (['a', 'b', 'c'], 'd', False),
    (['a', 'b', 'c'], 10, False),
    ([10, 20], 10, True),
...
def test_choice_type(choices, test_input, expected):
    ...
    assert str(config.Choice(*choices)) == '(' + ' or '.join([f"constant '{c}'" for c in choices]) + ')'
...
def test_dict_type(test_input, expected):
    ...
    assert str(config.Dict(config.Str, config.Int)) == 'dict with str keys and int values'
...
def test_and_type(test_input, expected):
    assert config.And(config.StrMatch(r'\d'), config.StrMatch(r'[!@#$%^&*]')).type_check(test_input) == expected
    assert str(config.And(config.StrMatch(r'\d'), config.StrMatch(r'[!@#$%^&*]'))) == "(str matching '\\d' and str matching '[!@#$%^&*]')"
