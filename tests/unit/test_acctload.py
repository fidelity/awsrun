...
@pytest.mark.parametrize('test_input, include, exclude, expected', [
    (
        ['100200300400'],
        {},
        {},
...
@pytest.mark.parametrize('acct_id', [
    '100200300400', '200300400100', '300400100200',
])
...
@pytest.mark.parametrize('str_template, expected', [
    (None, '10'),
    ('', '10'),
    ('hello', 'hello'),
    ('{id}', '10'),
...
@pytest.mark.parametrize('str_template', [
    '{this_key_is_not_in_the_dict}', '{id}/{neither_is_this_one}',
])
...
@pytest.mark.parametrize('include_attrs, expected', [
    (['id'],
     [{'id': '10'}]),
    (['id', 'bu'],
     [{'id': '10', 'bu': 'a'}]),
...
@pytest.mark.parametrize('id_attr, include_attrs', [
    (None, ['bu', 'env']),
    ('id', ['bu', 'env']),
    ('acctId', ['id', 'bu', 'env']),
])
def test_load_with_include_attrs_that_doesnt_have_the_id_attr(id_attr, include_attrs, one_acct_list):
    with pytest.raises(ValueError):
        acctload.MetaAccountLoader(one_acct_list, id_attr=id_attr, include_attrs=include_attrs)


@pytest.mark.parametrize('exclude_attrs, expected', [
    ([],
     [{'id': '10', 'bu': 'a', 'env': 'prd'}]),
    (['bu'],
     [{'id': '10', 'env': 'prd'}]),
...
@pytest.mark.parametrize('path, test_input, expected', [
    (None,
     [{'id': '10'}],
     [{'id': '10'}]),
    (['accounts'],
